...
let state = {
    ...
    hearts: parseInt(localStorage.getItem('rush80_hearts')) || 5, // CHANGED TO 5
...
function addScore() {
    const pts = 20; // CHANGED TO 20
    state.score += pts;
    Audio.sfx('score');

    state.coinBuffer += pts;
    while (state.coinBuffer >= 100) { // CHANGED TO 100
        state.coinBuffer -= 100;      // CHANGED TO 100
        state.coins += 5;             // CHANGED TO 5
        updateCoinHUD();
        saveData();
    }
...
function updateCoinHUD() {
    ...
    if(el) el.innerText = "COINS: " + state.coins;
}
