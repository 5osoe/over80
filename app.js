...
let state = {
    ...
    hearts: parseInt(localStorage.getItem('rush80_hearts')) || 3,
...
function updateHeartsUI(){
    ...
    if(el){
        el.innerText = "❤️ ".repeat(state.hearts);
    }
}
        ...
        for(let i = monsters.length - 1; i >= 0; i--){
            ...
            if(
                ...
                if(state.hearts <= 0){
                    ...
                    // Ensure UI updates for game over
...
function addScore() {
    ...
    while (state.coinBuffer >= 150) {
        state.coinBuffer -= 150;
        state.coins += 2;
        updateCoinHUD();
        saveData();
    }
...
window.startGame = () => {
    ...
    state.combatMode = false;
    state.bossActive = false;
    state.bossCount = 0; // CRITICAL FIX 2: Reset boss count
    state.monstersKilled = 0;
    monsters = [];
